 ...
 */
SeqQuickSort::SeqQuickSort(uint64_t size, float * data, int32_t * idList):Sorts(size, data, idList)
{
 ...
 * data
 */
void SeqQuickSort::quickSort(int32_t sIdx, int32_t eIdx, float * data, int32_t * idList)
{
  if (sIdx >= eIdx) return;

  //partition and return the index of the pivot
  int32_t mid = partition(sIdx, eIdx, data, idList);
 ...
 * eIdx - ending index into the data to sort
 * data - array of data to sort
 * idList - array of IDs in sequence
 * Modifies:
 * data
 * idList
 * Returns:
 * location of pivot
*/
int SeqQuickSort::partition(int32_t sIdx, int32_t eIdx, float * data, int32_t * idList)
{
